...
export class PlanHikeComponent implements OnInit {
  ...
  mapClicked(evt: any){
    ...
    if (this.allHaveValues(["endLat", "endLong", "startLat", "startLong"])) {
      this.geoService.getDistance(this.form.controls["startLat"].value, this.form.controls["startLong"].value, this.form.controls["endLat"].value, this.form.controls["endLong"].value).subscribe((data: any) => {
  ...
  allHaveValues(variables: string[]){
    for (const v of variables) {
      if (this.form.controls[v].value === null)
        return false;
    }
    return true;
  }
